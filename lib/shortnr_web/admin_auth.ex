defmodule ShortnrWeb.AdminAuth do
  use ShortnrWeb, :verified_routes
  ...
  @max_age 60 * 60 * 24 * 60
  @remember_me_cookie "_shortnr_web_admin_remember_me"
  ...
  def log_out_admin(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      ShortnrWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  the current_admin:
      ...
      defmodule ShortnrWeb.PageLive do
        use ShortnrWeb, :live_view

        on_mount {ShortnrWeb.AdminAuth, :mount_current_admin}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{ShortnrWeb.AdminAuth, :ensure_authenticated}] do
